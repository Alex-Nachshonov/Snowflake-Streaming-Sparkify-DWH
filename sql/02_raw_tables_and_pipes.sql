...
CREATE OR REPLACE TABLE UDACITY.STAGING.RAW_LOG_DATA (raw variant);

-- Copy song data from s3 into raw json table
CREATE OR REPLACE TABLE UDACITY.STAGING.RAW_SONG_DATA (raw variant);
...
CREATE OR REPLACE PIPE UDACITY.STAGING.LOG_DATA_PIPE AUTO_INGEST = TRUE AS COPY INTO UDACITY.STAGING.RAW_LOG_DATA
...
SELECT count(*)
FROM UDACITY.STAGING.RAW_LOG_DATA;

-- Define pipe for song data
CREATE OR REPLACE PIPE UDACITY.STAGING.SONG_DATA_PIPE AUTO_INGEST = TRUE AS COPY INTO UDACITY.STAGING.RAW_SONG_DATA
...
SELECT count(*)
FROM UDACITY.STAGING.RAW_SONG_DATA;
